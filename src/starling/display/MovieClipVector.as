...
package starling.display
{
    ...
    /** A MovieClip is a simple way to display an animation depicted by a list of Canvass.
     *
     *  <p>Pass the frames of the movie in a vector of Canvass to the constructor. The movie clip
     *  will have the width and height of the first frame. If you group your frames with the help
     *  of a Canvas atlas (which is recommended), use the <code>getCanvass</code>-method of the
     *  atlas to receive the Canvass in the correct (alphabetic) order.</p>
     *
     ...
     *
     *  @see starling.Canvass.CanvasAtlas
     */
    public class MovieClipVector extends Canvas implements IAnimatable
    {
        ...
        /** Creates a movie clip from the provided Canvass and with the specified default framerate.
         *  The movie will have the size of the first frame. */
        public function MovieClipVector(canvases:Vector.<Canvas>, fps:Number=12)
        {
            if (canvases.length > 0)
            {
                super();
                currentFrameCanvas = canvases[0];
                init(canvases, fps);
            }
            else
            {
        ...
        private function get currentFrameCanvas():Canvas { return _currentCanvas; }

        private function set currentFrameCanvas(currentFrameCanvas:Canvas):void
        {
            if(numChildren > 0)
                removeChild(_currentCanvas);
            _currentCanvas = currentFrameCanvas;
                                   ...
                                   duration:Number=-1):void
        {
            ...
            var frame:MovieClipFrame = new MovieClipFrame(canvas, duration);
            frame.sound = sound;
        ...
        public function advanceTime(passedTime:Number):void
        {
            ...
            if (previousFrameID != _currentFrameID)
                this.currentFrameCanvas = _frames[_currentFrameID].canvas;
...
import starling.display.MovieClipVector;

class MovieClipFrame
{
    public function MovieClipFrame(canvas:Canvas, duration:Number=0.1,  startTime:Number=0)
    {
        this.canvas = canvas;
    ...
    public function executeAction(movie:MovieClipVector, frameID:int):void
    {
        if (action != null)
        {
